...
MODEL_PATH=""
DEVICE="cuda"
# Defaults for finetuning specific goals
FINETUNE_CLASS_WEIGHTS="" # e.g., "1.0,1.5,2.0"
FINETUNE_MIN_PRECISIONS="0.9,0.9,0.9"
...
print_help() {
    ...
    echo "  --finetune_class_weights WEIGHTS  用于微调的类别权重，逗号分隔 (默认: \"\")"
    echo "  --finetune_min_precisions PRECISIONS 用于微调的各类别最小精确率目标，逗号分隔 (默认: \"0.9,0.9,0.9\")"
    echo ""
}
...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        --finetune_class_weights)
            FINETUNE_CLASS_WEIGHTS="$2"
            shift 2
            ;;
        --finetune_min_precisions)
            FINETUNE_MIN_PRECISIONS="$2"
            shift 2
            ;;
        *)
            echo "未知选项: $1"
...
# 日志目录
LOG_DIR="/root/autodl-tmp/eye_multimodal_ensemble/logs"
mkdir -p $LOG_DIR
...
case $MODE in
    train)
        ...
        # 训练命令
        TRAIN_CMD="python /root/autodl-tmp/eye_multimodal_ensemble/train.py \
            ...
            $MIXED_PRECISION_FLAG \
            $MODALITY_COMPLETION_FLAG \
            --output_dir /root/autodl-tmp/eye_multimodal_ensemble/train_results/${MODALITIES_STR}_${DATETIME}"
        
        echo "运行命令: $TRAIN_CMD"
        eval $TRAIN_CMD 2>&1 | tee $LOG_DIR/${MODALITIES_STR}_training_${DATETIME}.log
        ;;
    ...
    evaluate)
        ...
        EVAL_CMD="python /root/autodl-tmp/eye_multimodal_ensemble/evaluate.py \
            ...
            --output_dir /root/autodl-tmp/eye_multimodal_ensemble/test_results/${MODALITIES_STR}_${DATETIME}"
    ...
    finetune)
        ...
        CLASS_WEIGHTS_FLAG=""
        if [ ! -z "$FINETUNE_CLASS_WEIGHTS" ]; then
            CLASS_WEIGHTS_FLAG="--class_weights $FINETUNE_CLASS_WEIGHTS"
        fi
        
        # 微调命令
        FINETUNE_CMD="python /root/autodl-tmp/eye_multimodal_ensemble/train.py \
            --data_dir /root/autodl-tmp \
            --multimodal_index_path /root/autodl-tmp/eye_data_index/multimodal_dataset_index.json \
            --modalities $MODALITIES \
            --batch_size $BATCH_SIZE \
            ...
            $CLASS_WEIGHTS_FLAG \
            --min_precisions \"${FINETUNE_MIN_PRECISIONS}\" \
            --output_dir /root/autodl-tmp/eye_multimodal_ensemble/train_results/${MODALITIES_STR}_finetune_${DATETIME}"
        
        echo "运行命令: $FINETUNE_CMD"
        eval $FINETUNE_CMD 2>&1 | tee $LOG_DIR/${MODALITIES_STR}_finetune_${DATETIME}.log
        ;;
